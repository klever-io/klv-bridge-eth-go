...
type ethKleverBridgeComponents struct {
	...
	kcToEthMachineStates core.MachineStates
	kcToEthStepDuration  time.Duration
	kcToEthStatusHandler core.StatusHandler
	kcToEthStateMachine  StateMachine
...
func (components *ethKleverBridgeComponents) createDataGetter() error {
	kcDataGetterLogId := components.evmCompatibleChain.KleverBlockchainDataGetterLogId()
	argsKLVClientDataGetter := klever.ArgsKLVClientDataGetter{
		...
		Log:                     core.NewLoggerWithIdentifier(logger.GetOrCreate(kcDataGetterLogId), kcDataGetterLogId),
	}
...
func (components *ethKleverBridgeComponents) createKcClient(args ArgsEthereumToKleverBridge) error {
	...
	kcClientLogId := components.evmCompatibleChain.KleverBlockchainClientLogId()

	clientArgs := klever.ClientArgs{
		GasMapConfig:                 chainConfigs.GasMap,
		Proxy:                        args.Proxy,
		Log:                          core.NewLoggerWithIdentifier(logger.GetOrCreate(kcClientLogId), kcClientLogId),
	...
	components.kcClient, err = klever.NewClient(clientArgs)
	components.addClosableComponent(components.kcClient)
...
func (components *ethKleverBridgeComponents) createKleverRoleProvider(args ArgsEthereumToKleverBridge) error {
	...
	kcRoleProviderLogId := components.evmCompatibleChain.KleverBlockchainRoleProviderLogId()
	log := core.NewLoggerWithIdentifier(logger.GetOrCreate(kcRoleProviderLogId), kcRoleProviderLogId)
...
func (components *ethKleverBridgeComponents) createKcToEthereumBridge(args ArgsEthereumToKleverBridge) error {
	...
	configs, found := args.Configs.GeneralConfig.StateMachine[kcToEthName]
	if !found {
		return fmt.Errorf("%w for %q", errMissingConfig, kcToEthName)
	}

	components.kcToEthStepDuration = time.Duration(configs.StepDurationInMillis) * time.Millisecond
	argsTopologyHandler := topology.ArgsTopologyHandler{
		PublicKeysProvider: components.kleverRoleProvider,
		Timer:              components.timer,
		IntervalForLeader:  time.Second * time.Duration(configs.IntervalForLeaderInSeconds),
	...
	components.kcToEthStatusHandler, err = status.NewStatusHandler(kcToEthName, components.statusStorer)
	if err != nil {
		return err
	}

	err = components.metricsHolder.AddStatusHandler(components.kcToEthStatusHandler)
	if err != nil {
		return err
	}
	...
	components.kcToEthMachineStates, err = kctoeth.CreateSteps(bridge)
	if err != nil {
		return err
	}
...
func (components *ethKleverBridgeComponents) createKcToEthereumStateMachine() error {
	...
	var err error
	components.kcToEthStateMachine, err = stateMachine.NewStateMachine(argsStateMachine)
	if err != nil {
		return err
	}
	...
	argsPollingHandler := polling.ArgsPollingHandler{
		Log:              log,
		Name:             kcToEthName + " State machine",
		PollingInterval:  components.kcToEthStepDuration,
		PollingWhenError: pollingDurationOnError,
		Executor:         components.kcToEthStateMachine,
	}
